...
async function getHomeData() {
  const [categoriesRes, slidersRes, featuredRes, trendingRes, flashRes] = await Promise.allSettled([
    getCategories(),
    getSliders(),
    getFeaturedProducts(),
    getTrendingProducts(),
...
export default async function HomePage() {
  const { categories, sliders, featured, trending, flashSale } = await getHomeData();
    ...
    <main className="bg-[#F2F4F8] min-h-screen mt-2 mb-2">
      ...
      <section className="container-main pt-4 pb-5">
        <div className="flex gap-4">
          ...
          {/* LEFT: Category Sidebar — StarTech style */}
          <aside className="hidden lg:block w-[220px] flex-shrink-0">
            <div className="bg-white border border-[#e2e4e8]">
              <div className="bg-[#EF4A23] px-4 py-3">
                <span className="text-white text-sm font-bold uppercase tracking-wide">All Categories</span>
              </div>
